...
import type { DriftSignal, DesignToken, ColorValue, SpacingValue } from '../models/index.js';
...
describe('generateFixes', () => {
  ...
  describe('filtering', () => {
    it('filters by minimum confidence', () => {
      ...
      expect(fixes.every((f) => f.confidence === 'high')).toBe(true);
    });
    ...
    it('filters by file patterns', () => {
      const drifts = [
        createMockDrift('hardcoded-color', '#ff0000', 'src/components/Button.tsx:10:5'),
        createMockDrift('hardcoded-color', '#ff0000', 'src/pages/Home.tsx:20:3'),
      ];
      const tokens = [createColorToken('--color-danger', '#ff0000')];
