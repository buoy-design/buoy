...
import type { Component, DesignToken, DriftSignal, ColorValue, SpacingValue } from '@buoy-design/core';

// Helper to create mock component
function createMockComponent(name: string, framework: string = 'react'): Component {
  return {
    id: `component:${name}`,
    name,
    source: {
...
describe('generateContext', () => {
  ...
  describe('token section', () => {
    ...
    it('shows all tokens in comprehensive mode', () => {
      const result = generateContext(baseData, { detailLevel: 'comprehensive' });
  ...
  describe('detail levels', () => {
    ...
    it('comprehensive level produces longer output', () => {
      const standard = generateContext(baseData, { detailLevel: 'standard' });
      const comprehensive = generateContext(baseData, { detailLevel: 'comprehensive' });

      expect(comprehensive.content.length).toBeGreaterThan(standard.content.length);
    });
  });
